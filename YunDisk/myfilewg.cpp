...
void MyFileWg::uploadFilesAction()
{
    ...
    QString url = QString("http://%1:%2/md5").arg(QString(login->getIp())).arg(login->getPort());
...
void MyFileWg::uploadFile(UploadFileInfo *info)
{
    ...
    */
    ...
    QString url = QString("http://%1:%2/upload").arg(QString(login->getIp())).arg(login->getPort());
    request.setUrl(QUrl( url )); //设置url
...
void MyFileWg::refreshFileItems()
{
    ...
    if(m_fileList.isEmpty() == false)
    {
        ...
        for(int i = 0; i < n; ++i)
        {
            ...
            if (item == NULL) {
                cout << "NULLLLLLLLLLLLLLLLLLLL" << endl;
            }
//            ui->listWidget->addItem(new QListWidgetItem(QIcon(":/images/upload.png"), tmp->filename));
...
void MyFileWg::refreshFiles(MyFileWg::Display cmd)
{
    ...
    cout << "refreshFiles" << endl;

    // 获取登陆信息实例
    LoginInfoInstance *login = LoginInfoInstance::getInstance(); //获取单例

    // 127.0.0.1:80/myfiles&cmd=count		//获取用户文件个数
    QString url = QString("http://%1:%2/myfiles?cmd=count").arg(QString(login->getIp())).arg(login->getPort());
    request.setUrl(QUrl( url )); //设置url
...
void MyFileWg::getUserFilesList(MyFileWg::Display cmd)
{
    ...
    url = QString("http://%1:%2/myfiles?cmd=%3").arg(QString(login->getIp())).arg(login->getPort()).arg(tmp);
    request.setUrl(QUrl( url )); //设置url
...
void MyFileWg::getFileJsonInfo(QByteArray data)
{
    ...
    "share_status": 0,
    "pv": 0,
    "url": "http://192.168.230.138:80/group1/M00/00/00/wKgfbViy2Z2AJ-FTAaM3As-g3Z0782.mp4",
    "size": 27473666,
     "type": "mp4"
    }
    */
        ...
        if( doc.isObject())
        {
            ...
            for(int i = 0; i < size; ++i)
            {
                ...
                */
                ...
                cout << "info->filename: " << info->filename << endl;
                cout << "info->type :" << info->type << endl;
                cout << "QString type :" << type << endl;
...
void MyFileWg::shareFile(FileInfo *info)
{
    ...
    QString url = QString("http://%1:%2/dealfile?cmd=share").arg(QString(login->getIp())).arg(login->getPort());
    request.setUrl(QUrl( url )); //设置url
...
void MyFileWg::delFile(FileInfo *info)
{
    ...
    QString url = QString("http://%1:%2/dealfile?cmd=del").arg(QString(login->getIp())).arg(login->getPort());
    request.setUrl(QUrl( url )); //设置url
...
void MyFileWg::dealFilePv(QString md5, QString filename)
{
    ...
    QString url = QString("http://%1:%2/dealfile?cmd=pv").arg(QString(login->getIp())).arg(login->getPort());
    request.setUrl(QUrl( url )); //设置url
