...
void ShareList::refreshFiles()
{
    ...
    QString url = QString("http:// %1:%2/sharefiles?cmd=count").arg(QString(login->getIp())).arg(login->getPort());
...
void ShareList::getUserFilesList()
{
    ...
    QString url;
    url = QString("http:// %1:%2/sharefiles?cmd=normal").arg(QString(login->getIp())).arg(login->getPort());
    ...
    /*
        {
            ...
            "share_status": 0,
            "pv": 0,
            "url": "http://39.99.154.100:80/group1/M00/00/00/wKgfbViy2Z2AJ-FTAaM3As-g3Z0782.mp4",
            "size": 27473666,
            "type": "mp4"
        }
            -- user	文件所属用户
...
void ShareList::dealFilePv(QString md5, QString filename)
{
    ...
    QString url = QString("http:// %1:%2/dealsharefile?cmd=pv").arg(QString(login->getIp())).arg(login->getPort());
    request.setUrl(QUrl( url )); // 设置url
    cout << "url = " << url;
...
void ShareList::cancelShareFile(FileInfo *info)
{
    ...
    QString url = QString("http:// %1:%2/dealsharefile?cmd=cancel").arg(QString(login->getIp())).arg(login->getPort());
    request.setUrl(QUrl( url )); // 设置url
    cout << "url = " << url;
...
void ShareList::saveFileToMyList(FileInfo *info)
{
    ...
    QString url = QString("http:// %1:%2/dealsharefile?cmd=save").arg(QString(login->getIp())).arg(login->getPort());
    request.setUrl(QUrl( url )); // 设置url
    cout << "url = " << url;
